...
class Common(Cog):
    ...
    async def ping(self, ctx: Context) -> None:
        ...
        desc = textwrap.dedent(
            f"""
            :ping_pong: Pong!
            Bot's Latency: **{round(self.bot.latency * 1000)}ms**
            Bot's ping: **{duration}ms**
            Discord latency: **{discord_ms}**
            """
        )
    ...
    async def vote(self, ctx: Context, title: str, *options: str) -> None:
        ...
        embed = Embed(title=f"Vote Topic: {title}", description="\n\n".join(options.values()), color=Color.blue())
        message = await ctx.send(embed=embed)
    ...
    async def strawpoll(self, ctx: Context, *, question_and_choices: str = None) -> None:
        ...
        link = f"https://www.strawpoll.me/{strawpoll_id}"

        embed = Embed(
            title="Strawpoll!",
            description=f"Your strawpoll can be found here: [{link}]({link})",
            color=Color.blurple()
        )
